...
function Get-DebugLogsFromAPI {
    ...
    try {
        ...
        $response = Invoke-RestMethod -Uri "http://localhost:3000/api/debug-logs" -Method GET -ContentType "application/json"
        ...
        if ($response -and $response.logs) {
            ...
            Write-Host "No logs available yet, trying to trigger log collection..." -ForegroundColor Yellow
...
do {
    ...
    if ($logData) {
        ...
        if ($specificIssues.Count -gt 0) {
            Write-Host "`nSpecific issues found:" -ForegroundColor Yellow
        ...
        if ($hasErrors -or $specificIssues.Count -gt 0) {
            Write-Host "`nIssues detected! Please fix the problems and run again." -ForegroundColor Red
            Write-Host "Press any key to continue after fixing..." -ForegroundColor Yellow
            $null = $Host.UI.RawUI.ReadKey("NoEcho,IncludeKeyDown")
        } else {
            Write-Host "`nNo issues found! Application appears to be working correctly." -ForegroundColor Green
            break
        }
    } else {
        Write-Host "`nCould not retrieve debug logs. Please check if the server is running." -ForegroundColor Red
